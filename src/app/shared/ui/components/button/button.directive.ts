...
export type ButtonVariant = 'icon' | 'icon-accent' | 'pill' | 'primary';
...
const VARIANT_CLASSES: Record<ButtonVariant, string> = {
  ...
  // The filter pill, not the rounded-full category chip — that one is a link variant.
  pill: 'border-border text-ink-muted hover:text-ink flex h-11 shrink-0 cursor-pointer items-center gap-1.5 rounded-card border px-3 text-sm font-semibold',
  primary:
